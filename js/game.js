...
export default class UyghurTypingGame {
  ...
  prepareInputHandler() {
    ...
    this.keyHandler = (ev) => {
      ...
      // Limit buffer to last 2 characters
      if (this.inputBuffer.length > 2) {
        this.inputBuffer = this.inputBuffer.slice(-2);
      }
  
      const expected = this.seq[this.idx];
  
      // If buffer exactly matches the expected letter (e.g., 'ng' or 'a')
      if (this.inputBuffer === expected) {
        this.inputBuffer = '';
        this.handleCorrectKey();
        return;
      }
  
      // If the buffer is NOT a prefix of expected (e.g., 'nw' vs 'ng')
      if (!expected.startsWith(this.inputBuffer)) {
        this.inputBuffer = ''; // Reset buffer on mismatch
      }
    };
  }
